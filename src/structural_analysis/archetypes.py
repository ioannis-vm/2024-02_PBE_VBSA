...
def generate_archetype(
    ...
    for level_counter in range(num_levels):
        ...
        placement_df = pd.read_excel(
            f"data/{lateral_system}_{risk_category}_member_placement.xls",
    ...
    for key in range(1, 1 + num_levels):
        ...
        # This earlier version of the code was using tributary areas
        # to distribute loads to the beams.
        ...
        # We distribute the loads manually to get rid of the
        # scikit-geometry depenency which forces us to use
        # Anaconda. The resulting total mass for each story is the
        # same, so the change does not affect the dynamic response of
        # the building. The initial basic forces of the beams might be
        # a bit different, but this is not expected to cause any
        # reasonable deviation in the time-history analysis results
        # considering the model configuration we use.

        # get the level
        level = mdl.levels[key]
        # get the beams of that level
        comps = [
            comp
            for comp in level.components.values()
            if comp.component_purpose == "horizontal_component"
        ]
        # get the beamcolumn elements inside the beam component
        # assemblies
        beamcol_elms = []
        for comp in comps:
            comp_elms = list(comp.elements.values())
            for elm in comp_elms:
                if isinstance(elm, ElasticBeamColumn):
                    beamcol_elms.append(elm)
        # calculate the sum of the length of those beamcolumn
        # elements, to be used for load distribution
        total_length = sum(elm.clear_length() for elm in beamcol_elms)
        # calculate the total load from the slabs
        total_load = (
            (1.20 * surf_loads[key] + 0.25 * surf_loads_massless[key])
            * 150.00
            * 100.00
            * 12.00**2
        )  # lb
        total_mass = (
            (1.00 * surf_loads[key]) * 150.00 * 100.00 * 12.00**2 / G_CONST_IMPERIAL
        )  # lb / (in/s**2)
        # distribute the total load and mass to the beamcolumn
        # elements and their connecting nodes, using their clear
        # length as a proportionality measure
        for elm in beamcol_elms:
            uid = elm.uid
            udl = total_load / total_length
            loadcase.line_element_udl[uid].add_glob(np.array((0.00, 0.00, -udl)))
            mass = total_mass * (elm.clear_length() / total_length) / 2.00
            for node_uid in (elm.nodes[0].uid, elm.nodes[1].uid):
                loadcase.node_mass[node_uid].add(
                    np.array((mass, mass, mass, 0.00, 0.00, 0.00))
                )
    # done distributing slab loads and masses to the beams of each
    # level.
